#!/usr/bin/env python3
# sukisu_compat_fix.py — SukiSU 4.4 ARM64 manual-hook compatibility fixes
...
# ── Fix 10: patch_memory.c — insert compat AFTER includes, not before ─────
...
try:
    ...
    print('⚠️  patch_memory.c: file not found (skipping inline hook architecture setup)')

# ── Fix 11: copy_to_kernel_nofault → probe_kernel_write (added in 5.8) ────
...
# ── Fix 12: copy_from_user_nofault → copy_from_user (doesn't exist in 4.4)─
...
# ── Fix 13: __nocfi not defined in GCC 8 / 4.4 kernels ───────────────────
fix_all('__nocfi ', '', '__nocfi stripped')

# ── Fix 14: strncpy_from_user_nofault → strncpy_from_user (added in 5.8) ──
fix_all('strncpy_from_user_nofault', 'strncpy_from_user', 'strncpy_from_user_nofault → strncpy_from_user')

print("\n✅ All SukiSU 4.4 manual-hook compat fixes applied")
